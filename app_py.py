...
products = [
    "Apple iphone 15",
    "Apple 2023 MacBook Pro (16-inch, Apple M3 Pro chip with 12-core CPU and 18-core GPU, 36GB unified Memory, S",
    "Oneplus Nord 4 5G (Mercurial Silver, 8GB RAM, 256GB Storage)",
    "Sony WH-1000XMS Best Active Noise Cancelling Wireless Bluetooth Over Ear Headphones with Mic for Clear Call",
]
...
def analyze_sentiment (reviews):
    """Analyze customer sentiment for reviews."""
    sentiment_pipeline = pipeline("sentiment-analysis")
    return sentiment-pipeline(reviews)
...
forecast_df = pd.DataFrame({"Date": future_dates, "Predicted _Discount": forecast})
forecast_df.set_index("Date", inplace=True)
...
if not product_reviews.empty:
   ...
   st.subheader("Customer Sentiment Analysis")
   sentiment df = pd.DataFrame(sentiments)
   fig = px.bar(sentiment_df, x="label", title="Sentiment Analysis Results")
   st.plotly_chart(fig)
else:
    st.write("No reviews available for this product.")
